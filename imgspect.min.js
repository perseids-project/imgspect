...
jQuery.fn.cursorToEnd=function(){return this.each(function(){jQuery(this).focus();if(this.setSelectionRange){var b=2*jQuery(this).val().length;this.setSelectionRange(b,b)}else jQuery(this).val(jQuery(this).val());this.scrollTop=999999})};jQuery.fn.noSpace=function(){textNodes=this.contents().filter(function(){return 3==this.nodeType&&!/\S/.test(this.nodeValue)}).remove();return this};jQuery.fn.myHtml=function(){return jQuery(this).clone().wrap("<div>").parent().html()};
jQuery.fn.transLength=function(){var b=jQuery(this).css("transition").match(/ [\d|\.]+s/g);return 1E3*Number(b[0].replace("s",""))};String.prototype.smoosh=function(){return this.replace(/(\r\n+|\n+|\r+|\t+)/gm,"")};String.prototype.splice=function(b,c){return this.slice(0,Math.abs(c))+b+this.slice(Math.abs(c))};String.prototype.stripTags=function(){return this.replace(/<\/?[^>]+(>|$)/g,"")};String.prototype.oneSpace=function(){return this.replace(/\s{2,}/g," ")};
String.prototype.alphaSpaceOnly=function(){return this.replace(/[^\w\s]/gi,"")};String.prototype.alphaOnly=function(){return this.replace(/[^\w]/gi,"")};String.prototype.capitalize=function(){return this.charAt(0).toUpperCase()+this.slice(1)};String.prototype.repeat=function(b){return Array(b+1).join(this)};String.prototype.occurs=function(b,c){if(void 0==b)return this.length;b+="";if(0>=b.length)return this.length;for(var a=0,e=0,d=c?1:b.length;;)if(e=this.indexOf(b,e),0<=e)a++,e+=d;else break;return a};
String.prototype.positions=function(b,c,a,e){b+="";var d=0;c=c?1:b.length;for(var f=[];;){var g=!0,d=this.indexOf(b,d);if(0<=d){if(!0==a)for(var k=d;k<this.length&&"<"!=this[k];k++)">"==this[k]&&(g=!1);!0==e&&!0==this.substr(d-1,d+b.length+1).isAlphaNum()&&(g=!1);!0==g&&f.push(d);d+=c}else break}return f};String.prototype.insertAt=function(b,c){return this.substr(0,b)+c+this.substr(b)};
String.prototype.params=function(){for(var b=this.split("?"),b=b[1].split("&"),c={},a=0,e=b.length;a<e;a++)if(void 0!=b[a]){var d=b[a].split("=");c[d[0]]=d[1]}return c};String.prototype.hasUpper=function(){return/[A-Z]/.test(this)};String.prototype.report=function(){for(var b=this.toLowerCase().split(" "),c={},a=0,e=b.length;a<e;a++){var d=b[a];c[d]=d in c?c[d]+1:1}return c};String.prototype.lines=function(){return this.split("\n")};
String.prototype.isAlphaNum=function(){return/[^a-zA-Z0-9]/.test(this)?!1:!0};String.prototype.sentences=function(){for(var b=this.match(/[^\.!\?]+[\.!\?]+/g),c="aeiouy".split(""),a=[],e="",d=0;d<b.length;d++){for(var f=e+b[d],e="",g=!0,k=0;k<c.length;k++)if(-1!=f.indexOf(c[k])){g=!1;break}f.trim()[0].hasUpper()||(g=!0);g?0<a.length?a[a.length-1]+=f:e=f:a.push(f.smoosh().trim())}return a};
...
Culuh.prototype.intToHex=function(b){b=b.toString(16);2>b.length&&(b="0"+b);return b.toUpperCase()};Culuh.prototype.sat=function(b,c){var a=this.s,a=a*b;if(!0!=c)this.s=a,this.rgbUpdate();else return a=new Culuh(this.rgb()),a.sat(b),a};Culuh.prototype.toAlpha=function(b){b=parseFloat(b);b=0>b?0:b;return"rgba("+this.r+","+this.g+","+this.b+","+(1<b?1:b)+")"};
Culuh.prototype.invert=function(b){if(!0!=b)this.r=255-this.r,this.g=255-this.g,this.b=255-this.b,this.hsvUpdate();else return b=new Culuh(this.rgb()),b.invert(),b};
...
f.param.x2=a[d].coords[2]+f.param.x1;f.param.y2=a[d].coords[3]+f.param.y1;f.param.z=a[d].coords[4];var g=a[d].wipe,k=a[d].stay;b("img.star, .view",f.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});b(f.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});f.imgMove();f.viewCrop();b(".caption",f.elem).hide();var l=null;"caption"in a[d]&&(l=a[d].caption,"min"!=f.options.style||null!=f.options.style)&&(b(".caption",f.elem).show(),f.setCaption(l));d++;setTimeout(function(){f.sequence(a,
c,d)},1E3*(k+g));b(f.elem).trigger(f.events.frame,{caption:l})}};c.prototype.getToJson=function(a){if(""==a||void 0==a)return{};var b={};a=a.split("%");for(var c=0,f=a.length;c<f;c++){var g=a[c].split("=");2==g.length&&(b[g[0]]=decodeURIComponent(g[1].replace(/\+/g,"")))}return b};c.prototype.html=function(){var a=-1==this.src.indexOf("?")?"?":"&";return('<a class="imgbit" \t\t\t\t\t\thref="'+this.src+"\t\t\t\t\t\t"+a+"imgbit=\t\t\t\t\t\tx1="+this.param.x1+"\t\t\t\t\t\t%y1="+this.param.y1+"\t\t\t\t\t\t%x2="+
this.param.x2+"\t\t\t\t\t\t%y2="+this.param.y2+"\t\t\t\t\t\t%c="+this.param.c+"\t\t\t\t\t\t%z="+this.param.z+'">\t\t\t\t\t\t'+this.caption+"\t\t\t\t\t<a>").smoosh()};c.prototype.citeCoords=function(){var a=[];a[0]=this.param.x1/this.imgWidth;a[1]=this.param.y1/this.imgHeight;a[2]=(this.param.x2-this.param.x1)/this.imgWidth;a[3]=(this.param.y2-this.param.y1)/this.imgHeight;for(var b=0,c=a.length;b<c;b++)a[b]=a[b].toFixed(4);return a};c.prototype.toJsonLD=function(a){return{"@context":"http://www.w3.org/ns/oa-context-20130208.json",
"@type":"oa:Annotation",annotatedAt:this.timeCreated,annotatedBy:{"@type":"foaf:Person"},motivatedBy:"perseus:transcribing",label:"isQuotationOf"}};jQuery(document).ready(function(a){jQuery.fn.imgbit=function(a){var b=jQuery(this).selector;return this.each(function(){jQuery.data(this,b,new c(this,a,b))})}})})(jQuery);
