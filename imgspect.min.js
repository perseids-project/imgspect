...
this.param.z})};c.prototype.sequence=function(a,c,d){var f=this;d=void 0!=d?d:0;c=void 0==c?!1:c;if(a.length<=d)!0==c&&f.sequence(a,c,0);else{f.param.x1=a[d].coords[0];f.param.y1=a[d].coords[1];f.param.x2=a[d].coords[2];f.param.y2=a[d].coords[3];f.param.z=a[d].coords[4];var g=a[d].wipe,k=a[d].stay;b("img.star, .view",f.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});f.imgMove();f.viewCrop();d++;setTimeout(function(){f.sequence(a,c,d)},1E3*(k+g))}};c.prototype.getToJson=function(a){if(""==
a||void 0==a)return{};var b={};a=a.split("%");for(var c=0,f=a.length;c<f;c++){var g=a[c].split("=");2==g.length&&(b[g[0]]=decodeURIComponent(g[1].replace(/\+/g,"")))}return b};c.prototype.html=function(){var a=-1==this.src.indexOf("?")?"?":"&";return('<a class="imgbit" \t\t\t\t\t\thref="'+this.src+"\t\t\t\t\t\t"+a+"imgbit=\t\t\t\t\t\tx1="+this.param.x1+"\t\t\t\t\t\t%y1="+this.param.y1+"\t\t\t\t\t\t%x2="+this.param.x2+"\t\t\t\t\t\t%y2="+this.param.y2+"\t\t\t\t\t\t%c="+this.param.c+"\t\t\t\t\t\t%z="+
this.param.z+'">\t\t\t\t\t\t'+this.caption+"\t\t\t\t\t<a>").smoosh()};c.prototype.citeCoords=function(){var a=[];a[0]=this.param.x1/this.imgWidth;a[1]=this.param.y1/this.imgHeight;a[2]=(this.param.x2-this.param.x1)/this.imgWidth;a[3]=(this.param.y2-this.param.y1)/this.imgHeight;for(var b=0,c=a.length;b<c;b++)a[b]=a[b].toFixed(4);return a};c.prototype.toJsonLD=function(a){return{"@context":"http://www.w3.org/ns/oa-context-20130208.json","@type":"oa:Annotation",annotatedAt:this.timeCreated,annotatedBy:{"@type":"foaf:Person"},
motivatedBy:"perseus:transcribing",label:"isQuotationOf"}};jQuery(document).ready(function(a){jQuery.fn.imgbit=function(a){var b=jQuery(this).selector;return this.each(function(){jQuery.data(this,b,new c(this,a,b))})}})})(jQuery);
