...
e.param.x2=b[d].coords[2];e.param.y2=b[d].coords[3];e.param.z=b[d].coords[4];var g=b[d].wipe,h=b[d].stay;a("img.star, .view",e.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});a(e.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});e.imgMove();e.viewCrop();a(".caption",e.elem).hide();var k=null;"caption"in b[d]&&(k=b[d].caption,"min"!=e.options.style||null!=e.options.style)&&(a(".caption",e.elem).show(),e.setCaption(k));d++;setTimeout(function(){e.sequence(b,
...
jQuery.fn.cursorToEnd=function(){return this.each(function(){$(this).focus();if(this.setSelectionRange){var a=2*$(this).val().length;this.setSelectionRange(a,a)}else $(this).val($(this).val());this.scrollTop=999999})};jQuery.fn.myHtml=function(){return $(this).clone().wrap("<div>").parent().html()};jQuery.fn.transLength=function(){var a=$(this).css("transition").match(/ [\d|\.]+s/g);return 1E3*Number(a[0].replace("s",""))};
String.prototype.smoosh=function(){return this.replace(/(\r\n+|\n+|\r+|\t+)/gm,"")};String.prototype.occurs=function(a,c){if(void 0==a)return this.length;a+="";if(0>=a.length)return this.length;for(var b=0,f=0,d=c?1:a.length;;)if(f=this.indexOf(a,f),0<=f)b++,f+=d;else break;return b};String.prototype.params=function(){for(var a=this.split("?"),a=a[1].split("&"),c={},b=0,f=a.length;b<f;b++)if(void 0!=a[b]){var d=a[b].split("=");c[d[0]]=d[1]}return c};String.prototype.hasUpper=function(){return/[A-Z]/.test(this)};
