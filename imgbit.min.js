...
this.param.z})};c.prototype.sequence=function(b,c,d){var e=this;d=void 0!=d?d:0;c=void 0==c?!1:c;if(b.length<=d)!0==c&&e.sequence(b,c,0);else{e.param.x1=b[d].coords[0];e.param.y1=b[d].coords[1];e.param.x2=b[d].coords[2];e.param.y2=b[d].coords[3];e.param.z=b[d].coords[4];var g=b[d].wipe,h=b[d].stay;a("img.star, .view",e.elem).css({transition:"all "+g+"s","-webkit-transition":"all "+g+"s"});e.imgMove();e.viewCrop();d++;setTimeout(function(){e.sequence(b,c,d)},1E3*(h+g))}};c.prototype.getToJson=function(a){if(""==
a||void 0==a)return{};var c={};a=a.split("%");for(var d=0,e=a.length;d<e;d++){var g=a[d].split("=");2==g.length&&(c[g[0]]=decodeURIComponent(g[1].replace(/\+/g,"")))}return c};c.prototype.html=function(){var a=-1==this.src.indexOf("?")?"?":"&";return('<a class="imgbit" \t\t\t\t\t\thref="'+this.src+"\t\t\t\t\t\t"+a+"imgbit=\t\t\t\t\t\tx1="+this.param.x1+"\t\t\t\t\t\t%y1="+this.param.y1+"\t\t\t\t\t\t%x2="+this.param.x2+"\t\t\t\t\t\t%y2="+this.param.y2+"\t\t\t\t\t\t%c="+this.param.c+"\t\t\t\t\t\t%z="+
